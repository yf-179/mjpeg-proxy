...
use hyper::{self as http, body::Bytes, header::{CONTENT_LENGTH, CONTENT_TYPE}, server::conn::AddrStream, service::{make_service_fn, service_fn}, Body, HeaderMap, Request, Response};
use lazy_static_include::lazy_static_include_bytes;
use multipart_stream::Part;
use tokio::{sync::{broadcast, watch}, time::{interval, sleep}};
...
impl Server {
    ...
    pub async fn run(&self) {
        ...
        for v in self.streams.iter() {
            Arc::get_mut(&mut stream_config).unwrap().insert(v.1.path.clone(), Arc::clone(v.1.broadcaster.as_ref().unwrap()));
